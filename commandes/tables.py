...
class CommandeTable(tables.Table):
    ...
    class Meta:
        model = Commandes
        sequence = ('id','created_at','numero_commande','price','commission','client','etat','driver','actions')

        exclude = {'colis','accepted','status','observation','insurance','updated_at','date_depot', 'date_reception', 'city_depart', 'city_arrive', 'package','agent'}
...
class CommandeClientTable(tables.Table):
    ...
    numero_commande = tables.LinkColumn("commandes:mes_commandes_detail", args=[A("numero_commande")])

    actions  = TemplateColumn(template_name="commandes/includes/commande_client_actions.html", attrs={"td": {"class": "text-right"}})

    class Meta:
        model = Commandes
        sequence = ('numero_commande','colis','city_depart', 'city_arrive','price','etat','actions')
        exclude = {'status', 'accepted','id','commission','observation','insurance','updated_at', 'date_depot', 'date_reception', 'created_at', 'package','agent','driver'}
        template_name = "django_tables2/bootstrap.html"
        attrs = {
	        "th" : {
	            "_ordering": {
